...
// Sets default values
AGATraversalBlock::AGATraversalBlock(){
	...
	StaticMeshComponent = CreateDefaultSubobject<UStaticMeshComponent>(FName(TEXT("Mesh")));
	
	StaticMeshComponent->AttachToComponent(SceneComponent, FAttachmentTransformRules::KeepRelativeTransform);
}

// Called when the game starts or when spawned
void AGATraversalBlock::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AGATraversalBlock::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

USplineComponent* AGATraversalBlock::FindClosestLedgeToActor(const FVector& ActorLocation){
...
void AGATraversalBlock::GetLedgeTransform(FVector HitLocation, FVector ActorLocation, FTraversalCheckResult& TraversalTraceResultInOut)
{
